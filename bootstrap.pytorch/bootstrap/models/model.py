...
class Model(nn.Module):
    ...
    def cuda(self, device_id=3):
        ...
        """
        self.is_cuda = True
        return self._apply(lambda t: self.cuda_tf()(t))
    ...
    def state_dict(self, *args, **kwgs):
        ...
        for mode, criterion in self.criterions.items():
            if isinstance(criterion, nn.Module) and  hasattr(criterion, '_parameters'):
                try:
                    state['criterions'][mode] = criterion.state_dict(*args, **kwgs)
                except AttributeError:
                    state['criterions'][mode] = {}
        state['metrics'] = {}
        for mode, metric in self.metrics.items():
            if isinstance(metric, nn.Module) and hasattr(metric, '_parameters'):
                try:
    ...
    def load_state_dict(self, state, *args, **kwgs):
        """
        """
        msg = self.network.load_state_dict(state['network'], *args, **kwgs)
        print(msg)
        for mode, criterion in self.criterions.items():
            if isinstance(criterion, nn.Module) and  hasattr(criterion, '_parameters') and state['criterions'][mode]:
                criterion.load_state_dict(state['criterions'][mode], *args, **kwgs)
        for mode, metric in self.metrics.items():
            if isinstance(metric, nn.Module) and mode in state['metrics'] and  hasattr(metric, '_parameters') and state['metrics'][mode]:
