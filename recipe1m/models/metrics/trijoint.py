...
def print_trainable_params_percentage(model):
    ...
    return trainable_size, orig_param_size
...
class Trijoint(nn.Module):
    ...
    def __init__(self, opt, with_classif=False, engine=None, mode='train'):
        ...
        Logger()('self.save_ids', self.save_ids)
    ...
    def calculate_metrics(self):
        ########## 
        Logger()('model footprint...')
        # Logger()(self.engine.model)
        trainable_size, orig_param_size = print_trainable_params_percentage(self.engine.model)
        percentage = trainable_size / orig_param_size * 100
        Logger()(f"Trainable param percentage: {percentage:.2f}% ({trainable_size}/{orig_param_size})")
        ...
        for i in range(final_nb_bags):
            ...
            for i in range(nb_identifiers_image):
                pos_im2recipe = recipe2im[i,:].tolist().index(i) 
                pos_recipe2im = im2recipe[:,i].tolist().index(i)
