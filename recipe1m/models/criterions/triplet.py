...
class Triplet(nn.Module):
    ...
    def __init__(self, opt, nb_classes, dim_emb, engine=None):
        ...
        self.gather_embeddings = opt['retrieval_strategy'].get('gather_embeddings', False)
    ...
    def semantic_multimodal(self, distances, class1, class2, erase_diagonal=True):
        ...
        matrix_mask = torch.logical_and((class1_matrix != 0), (class2_matrix != 0))
    ...
    def __call__(self, input1, input2, target, class1, class2):
        ...
        if len(set(['SIRR', 'SRII']).intersection(self.substrategy)) > 0:
            ...
            matrix_mask = torch.logical_and((semantic_class1 != 0), (semantic_class2 != 0))
            if 'SIRR' in self.substrategy:
                if matrix_mask.size()[0] == 0:
                    cost = distances.sum()*0
                else:
                    cost = self.calculate_cost(self.semantic_multimodal(distances, semantic_class1, semantic_class2))
                self.add_cost('SIRR', cost, bad_pairs, losses)

            if 'SRII' in self.substrategy:
                if matrix_mask.size()[0] == 0:
                    cost = distances.sum()*0
                else:
                    cost = self.calculate_cost(self.semantic_multimodal(distances.t(), semantic_class2, semantic_class1))
                self.add_cost('SRII', cost, bad_pairs, losses)
