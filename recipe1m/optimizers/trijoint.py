...
class Trijoint(torch.optim.Optimizer):
    ...
    def __init__(self, opt, model, engine=None):
        ...
        self.optimizer_bert = opt.get('optimizer_bert', False)

        if self.optimizer_bert:
            self.lr_bert = opt.get('lr_bert', self.lr)
            self.optimizers['bert'] = torch.optim.Adam(self.model.network.get_parameters_bert(), self.lr_bert)
    ...
    def _activate_model(self):
        ...
        activate_recipe = (optim_name == 'recipe') or (optim_name == 'recipe&image') or (optim_name == 'recipe&image&bert')
        activate_image = (optim_name == 'image') or (optim_name == 'recipe&image') or (optim_name == 'recipe&image&bert')
        activate_bert = optim_name == 'recipe&image&bert'
        activate_cross = True
        ...
        if not self.freeze_im:
            for p in self.model.network.get_parameters_image():
                try:
                    ...
                    for p_ in p['params']:
                        p_.requires_grad = activate_image
        ...
        if self.optimizer_bert:
            for p_dict in self.model.network.get_parameters_bert():
                for p in p_dict['params']:
                    p.requires_grad = activate_bert
    ...
    def _auto_fixed_fine_tune(self):
        if self.current_optimizer_name == 'recipe' and self.epoch == self.switch_epoch:
            ...
            if self.optimizer_bert:
                self.current_optimizer_name = 'recipe&image&bert'
            self.freeze = False
